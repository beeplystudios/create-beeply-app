 ...
 * @link https://github.com/t3-oss/create-t3-app/blob/main/cli/src/helpers/git.ts
 *
...
export const initializeGit = async (projectDir: string) => {
  ...
  try {
    ...
    spinner.succeed(
      `${chalk.green("Successfully initialized and staged")} ${chalk.greenBright.bold(
        "git"
      )}\n`
    );
  } catch (error) {
